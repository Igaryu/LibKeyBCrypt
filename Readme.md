...
## `GenKeyFile(NomeFile)`
L’utente deve dare in input la password che vuole configurare. **Se la password è minore in lunghezza di 8 caratteri** non viene accettata, e viene restituito al chiamante il valore -2. **Se la password è di 8 o più caratteri**, viene creato, o sovrascritto, un ***NomeFile*** contenente una chiave generata da `bcrypt` per il successivo confronto durante gli accesi, e alla fine, viene chiuso il file. Viene tornato al chiamante il valore 0 (zero)
